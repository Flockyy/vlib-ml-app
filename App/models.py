...
def init_db():
    db.drop_all()
    db.create_all()
    Users(last_name = 'Charlie', first_name='Florian', username='FloCha', password_hash=generate_password_hash('1234', method='sha256')).save_to_db()
    Predicitions(user_id=1, Datetime="08/03/2022 10:00:00", Season="Spring", Weather="Clear", Workday=True, Temperature=20.0, Atemperature=19.2,Humidity = 70, Windspeed=70).save_to_db()
    
    lg.warning('Database initialzed')